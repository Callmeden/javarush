...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        for(Map.Entry<String,Double> pair:map.entrySet()){
            if (pair.getValue()>maxvalue) maxvalue=pair.getValue(); //ищем максимальную зп
        }
        for(Map.Entry<String,Double> pair:map.entrySet()){
            if(pair.getValue()==maxvalue)System.out.println(pair.getKey()); //выводим пару если максимальная зп
        }
    }
}
